...
lib.create = (dir, file, data, callback) => {
  ...
  fs.open(`${lib.basedir + dir}/${file}.json`, 'wx', (err, fileDescriptor) => {
    // 'wx' is a flag which checks if the file to be written already exists or not. If it exists, file cannot be written with the same name
...
lib.update = (dir, file, data, callback) => {
  fs.open(`${lib.basedir + dir}/${file}.json`, 'r+', (err, fileDescriptor) => {
    // 'r+' is a flag which both read and write file, also checks if that file exists or not. If it does not exist, throws an error
    if (!err && fileDescriptor) {
      // Truncate the file
